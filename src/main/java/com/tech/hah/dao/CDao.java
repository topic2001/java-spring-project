...
public class CDao {
	...
	public ArrayList<CAccommodationDto> map(String[] accList) throws Exception {
		...
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sql = "select * from(select a.*,b.room_price,row_number() over(partition by a.acc_code order by b.room_price desc)as "
				+ "rnum from accommodation a,room b where a.acc_code=b.acc_code and a.acc_code=?) where rnum=1";
	...
	public void res_cancel(String res_code) throws Exception {
		Context context = new InitialContext();
		DataSource dataSource = (DataSource) context.lookup("java:comp/env/jdbc/springorcl");
		Connection con = dataSource.getConnection();
		PreparedStatement pstmt = null;
		
		String sql = "update reserve set r_status = '예약 취소' where res_code = ? ";
		pstmt = con.prepareStatement(sql);
	...
	public void add_mainImg(String acc_code, String reName, int i) throws Exception {
		Context context = new InitialContext();
		DataSource dataSource = (DataSource) context.lookup("java:comp/env/jdbc/springorcl");
		Connection con = dataSource.getConnection();
		PreparedStatement pstmt = null;
		...
		pstmt = con.prepareStatement(sql);
		pstmt.setInt(1, Integer.parseInt(acc_code));
		pstmt.setString(2, photo_type);
	...
	public void add_room(String acc_code, String room_type, String room_price) throws Exception {
		Context context = new InitialContext();
		DataSource dataSource = (DataSource) context.lookup("java:comp/env/jdbc/springorcl");
		Connection con = dataSource.getConnection();
		PreparedStatement pstmt = null;
		...
		pstmt.setInt(3, Integer.parseInt(room_price));
		pstmt.executeUpdate();
	...
	public void add_roomImg(String acc_code, String room_type, String reName) throws Exception {
		Context context = new InitialContext();
		DataSource dataSource = (DataSource) context.lookup("java:comp/env/jdbc/springorcl");
		Connection con = dataSource.getConnection();
		PreparedStatement pstmt = null;
