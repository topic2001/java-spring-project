...
public class CController {
	...
	@RequestMapping("/reserve_cancel")
	public String reserve_cancel(HttpServletRequest request) throws Exception {
		String res_code = request.getParameter("res");
		CDao dao = new CDao();
		dao.res_cancel(res_code);
	...
	@RequestMapping(value = "/reply_send", produces = "application/text; charset=utf8")
	@ResponseBody
	public void reply_send(HttpServletRequest request) {
		String email = request.getParameter("email");
		...
		String setfrom = "hahtestid@gmail.com";
		String email_receiver = request.getParameter("email"); // 받는 사람 이메일
		String title = "저기어때 문의 답변 메일입니다"; // 제목
		String content = content_; // 내용
	...
	@RequestMapping(value = "/main_photo", method = RequestMethod.POST)
	public String uploadFile(MultipartFile[] upload, HttpServletRequest request) throws Exception { 
		String acc_code = request.getParameter("acc_code");
		String saveDir = request.getSession().getServletContext().getRealPath("/resources/img/acc_img");
		CDao dao = new CDao();
		String reName = "";
		int i = 1;
		for(MultipartFile f : upload) { 
			if(!f.isEmpty()) { 
				...
				// 파일 이름 변경 
				reName = acc_code+"_MAIN_"+i+sdf.format(System.currentTimeMillis()) + "_" + rand + ext; 
				
				// 파일 저장
				try {
					f.transferTo(new File(saveDir + "/" + reName)); 
					dao.add_mainImg(acc_code,reName,i);
					i++;
					}catch (IOException e) { 
						e.printStackTrace();
						} 
		...
		return "redirect:kaccommodation";
			
		
		}	
	
	@RequestMapping(value = "/room_write", method = RequestMethod.POST)
	public String room_write(MultipartFile[] upload, HttpServletRequest request) throws Exception { 
		String acc_code = request.getParameter("acc_code");
		...
		for(MultipartFile f : upload) { 
			if(!f.isEmpty()) { 
				...
				// 파일 이름 변경 
				reName = acc_code+"_"+room_type+"_"+sdf.format(System.currentTimeMillis()) + "_" + rand + ext; 
				
				// 파일 저장
				try {
					f.transferTo(new File(saveDir + "/" + reName)); 
					dao.add_room(acc_code,room_type,room_price);
					dao.add_roomImg(acc_code,room_type,reName);
					}catch (IOException e) { 
						e.printStackTrace();
						} 
				} 
